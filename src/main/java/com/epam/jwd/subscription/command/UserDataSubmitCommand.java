...
public class UserDataSubmitCommand implements Command {
    ...
    private static final String ERROR_USER_DATA_ATTRIBUTE = "errorUserDataMessage";
    ...
    public CommandResponse execute(CommandRequest request) {
        ...
        int age;
        if (request.getParameter(AGE_REQUEST_PARAM_NAME).equals(EMPTY_STRING)) {
            ...
            age = Integer.parseInt(request.getParameter(AGE_REQUEST_PARAM_NAME));
        }
        ...
        if (UserDataValidator.getInstance().validateAll(firstName, lastName, age, email)) {
            if (userService.readUserByAccountId(accId).isPresent()) {
                userService.updateByAccountId(firstName, lastName, age, email, accId);
            } else {
    ...
    private Long getAccountId (CommandRequest request) {
        if (request.retrieveFromSession(ACCOUNT_SESSION_ATTRIBUTE_NAME).isPresent()) {
           ...
           request.addAttributeToJsp(ERROR_USER_DATA_ATTRIBUTE, ERROR_SESSION_DOES_NOT_EXIST_MESSAGE);
